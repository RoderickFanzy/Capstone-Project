...
# partition
set.seed(1)
spot.df <- spot.clean[,-c(1,2,8)]
train.index <- sample(c(1:dim(spot.df)[1]), dim(spot.df)[1]*0.6)
train <- spot.df[train.index,]
valid <- spot.df[-train.index,]

# classification tree
#spot.ct <- rpart(End.Status ~., data = train, method="class")

# plot tree
#prp(spot.ct, type=1, extra=1, under=TRUE, split.font=1, varlen= -10)

#spot.pred <- predict(spot.ct, valid, type="class")
#confusionMatrix(spot.pred, as.factor(valid$End.Status))
