...
box.gap.3 <- ggplot(data = spot.gap.3)+
                geom_boxplot(aes(x = Patient.Sex, y = time.gap3, fill = Patient.Sex), alpha = 0.5)+
                coord_flip()+
                theme_classic()
...
ggplot(test.rate, aes(x = reorder(Client.ID,resulted_rate), y = resulted_rate, fill = Client.ID)) +
  geom_bar(aes(y = Count),stat = "identity", alpha = 0.5, width = 0.5) +
  geom_text(aes(label = paste0(Count, " / ",round(resulted_rate*100,2), "%"), y = Count + 1), 
            color = "black", fontface = "bold", size = 4, hjust = -0.1) +
  labs(x = "", y = "Test times (with Success Rate)", 
...
# random forest
spot.rf <- randomForest(as.factor(End.Status) ~ ., data = spot.df, ntree = 200, mtry=4, nosize=5, importance=TRUE)
...
mixed.model <- glm(End.Status ~  as.factor(Specimen.Type)
                                  +  time.gap1
                                  +  time.gap2
                                  +  Patient.Sex
                                  +  Patient.Age,
...
test_predictions_binary <- ifelse(test_predictions > 0.85, 1, 0)
...
table(spot.km$Rejection.Reason)



rej_reason_freq <- as.data.frame(table(spot.km$Rejection.Reason))
colnames(rej_reason_freq) <- c("Reject Reason","Count")

ggplot(rej_reason_freq, aes(x = reorder(`Reject Reason`,Count), y = Count, fill = `Reject Reason`)) + 
  geom_bar(stat = "identity", alpha = 0.5) +
  labs(x = "Rejection Reason", y = "Count") +
  theme_classic()+
  coord_flip()
```
