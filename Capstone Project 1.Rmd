...
threshold.duration <-  1.5*(IQR(test.duration$n))
outliers.duration <- test.duration$n < median(test.duration$n) - threshold.duration | test.duration$n > median(test.duration$n) + threshold.duration
test.duration <- test.duration[!outliers,]

test.duration$n <- as.factor(test.duration$n)

ggplot(test.duration, aes(x = n)) +
  geom_bar( alpha = 0.5, color = "black", fill = "cornflowerblue") +
